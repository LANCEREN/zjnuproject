...
from Influence_Prediction.utils import create_networkx_graph, static_network_partition, dynamic_network_partition, \
...
class TimeWindowSplitter:
    ...
    def split_by_time(self, post_info_list, follower_network):
        """按时间窗口划分转发关系网络"""
        # 定义日期范围
        min_time = min([datetime.datetime.strptime(post.publish_time, "%Y%m%d%H%M%S").date() for post in post_info_list])
        max_time = max([datetime.datetime.strptime(post.publish_time, "%Y%m%d%H%M%S").date() for post in post_info_list])
        ...
        for post in post_info_list:
            if not post.is_original:
                date = datetime.datetime.strptime(post.publish_time, "%Y%m%d%H%M%S").date()
                if date <= mid_time:
...
class DataProcessor:
    ...
    def load_networks_with_time_txt(self, static_network: str, dynamic_network: str, idx_map) -> networkx:
...
class NodeEmbedding:
    ...
    def generate_embeddings(self, G: networkx, embedding_dim: int) -> np.ndarray:
        """生成节点嵌入"""
        # 实例化 LINE 模型 设置模型参数
        model = LINE(dimension=embedding_dim, walk_length=20, walk_num=20, negative=10, batch_size=64, alpha=0.01, order=2)
...
class FeatureExtractor:
    def extract_group_static_features(self, static_network: networkx, dynamic_network: networkx, k: int) -> np.ndarray:
        """提取节点的群体特征"""
        average_perceptions = []
        fs = []
        nodes = list(sorted(static_network.nodes))
        for i in tqdm(nodes, desc="Processing nodes"):
            average_perception, f = static_network_partition(static_network, dynamic_network, target_user=i, top_k=k)
            average_perceptions.append(average_perception)
            fs.append(f)
        # 将节点编号、平均感知度和 f 值存储为 NumPy 数组
        node_data = np.array([(node, perception, f_value) for node, perception, f_value in
                              zip(nodes, average_perceptions, fs)])
        return node_data
    def extract_group_dynamic_features(self, dynamic_network : networkx, k : int) -> np.ndarray:
        """提取节点的群体特征"""
        finals = []
        nodes = list(sorted(dynamic_network.nodes))
        for i in tqdm(nodes, desc="Processing nodes"):
            final = dynamic_network_partition(dynamic_network, np.array(dynamic_network.edges()), i, k)
            finals.append(final)
        # 将节点编号、 f 值存储为 NumPy 数组
        node_data = np.array([(node, f_value) for node, f_value in
                              zip(nodes, finals)])
        return node_data
