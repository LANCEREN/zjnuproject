...
from src.SJTU.DDQN.model import FinetuningGAT
from src.SJTU.Interface import AccountInfo, DDQNInterface, PostInfo
...
def json_preprocess_sansuo():
    ...
    # 保存整理后的数据到新的JSON文件
    output_file_path = (
        dataPath.data_sansuo_directory
        + "./processed_posts_info_with_missing_"
        + file_name
    )
...
def json_selection_process_sansuo():
    ...
    for i in range(len(df["userid"])):
        userid_list.append(
            "None" if pd.isnull(df["userid"][i]) else int(df["userid"][i])
        )
        relevant_userid_list.append(
            "None"
            if pd.isnull(df["relevant_userid"][i])
            else int(df["relevant_userid"][i])
        )
...
def json_preprocess_zjnu(
    budget: int, account_info_list_file_name: str, post_info_list_file_name: str
):
    account_info_list_file_data = readJSONdata(
        account_info_list_file_name, dataPath.data_zjnu_directory
    )
    post_info_list_file_data = readJSONdata(
        post_info_list_file_name, dataPath.data_zjnu_directory
    )
    ...
    for account_info_dict in account_info_list_file_data:
        ...
        if "user_embeddings" in account_info_dict:
            account_info_dict["user_embeddings"] = torch.tensor(
                account_info_dict["user_embeddings"]
            )
    ...
    # 创建 DDQNInterface 实例
    ddqn_interface = DDQNInterface(
        budget=100,
        account_info_list=account_info_list,
        post_info_list=post_info_list,
        user_map_dict=user_dict,
        user_dict_reverse=user_dict_reverse,
    )
