        ...
        while True:
            ...
            while len(queue) != 0:
                ...
                for child in graph.get_children(curr_node):
                    if not (
                        child in seed_nodes_set
                    ) and random.random() <= graph.get_node_pro(child):
...
def computeMC(graph, S, R):
    ...
    for _ in range(R):
        ...
        while True:
            ...
            while len(queue) != 0:
                curr_node = queue.popleft()
                curr_source_set.update(
                    child
                    for child in graph.get_children(curr_node)
                    if not (child in source_set)
                    and random.random() <= graph.edges[(curr_node, child)]
                )
            if len(curr_source_set) == 0:
                break
            queue.extend(curr_source_set)
...
def computeRR(graph, S, R, cache=None):
    ...
    if cache is not None:
        if len(cache) > 0:
            ...
            return (
                sum(any(s in RR for s in S) for RR in cache)
                * 1.0
                / R
                * graph.num_nodes()
            )
    ...
    for i in range(R):
        ...
        while True:
            ...
            while len(queue) != 0:
                ...
                curr_source_set.update(
                    parent
                    for parent in graph.get_parents(curr_node)
                    if not (parent in source_set)
                    and random.random() <= graph.edges[(parent, curr_node)]
                )
            if len(curr_source_set) == 0:
                break
            queue.extend(curr_source_set)
