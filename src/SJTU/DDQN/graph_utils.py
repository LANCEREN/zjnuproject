...
class Graph:
    def __init__(self, nodes, edges, children, parents, node_feature_path):
        ...
        external_features_dict = {
            feature_dict["account_id"]: feature_dict
            for feature_dict in external_features
        }

        # 获取 personal_desc 的最大长度
        max_personal_desc_length = max(
            len(feature_dict.get("personal_desc", []))
            for feature_dict in external_features
        )
        ...
        for node in self.G.nodes():
            ...
            if node in external_features_dict:
                ...
                if len(personal_desc) < max_personal_desc_length:
                    features.extend(
                        [0] * (max_personal_desc_length - len(personal_desc))
                    )
            else:
    ...
    def get_node_features(self):
        ...
        node_features = np.array(
            [self.G.nodes[node]["features"] for node in self.G.nodes()]
        )
...
def preprocess_graph(file_path=None, ori_feature_path=None, output_dir=None):
    # 先处理图
    graph_path, _, node_feature_path = process_graph(
        file_path, ori_feature_path, output_dir
    )
    # 再按照平台进行划分
...
def process_graph(file_path=None, node_feature_path=None, output_dir=None):
    ...
    for feature_dict in external_features:
        ...
        if (
            isinstance(old_account_id, str) and old_account_id.strip()
        ):  # 检查 old_account_id 是否为非空字符串
            try:
    ...
    if len(parts) == 3:
        new_edge_list = [
            (node_mapping[node1], node_mapping[node2], timestamp)
            for node1, node2, timestamp in edge_list
        ]
    elif len(parts) == 2:
        new_edge_list = [
            (node_mapping[node1], node_mapping[node2]) for node1, node2 in edge_list
        ]
    # 保存新的图数据
    ...
    if dataset_name == "sansuo":
        ...
        with open(graph_path, "w", newline="") as csv_file:
            ...
            for post in post_features:
                if (
                    post["userid"] and post["relevant_user_id"]
                ):  # 检查 userid 和 relevant_user_id 是否为空
                    try:
                        csv_writer.writerow(
                            [int(post["userid"]), int(post["relevant_user_id"])]
                        )
                    except ValueError as e:
                        print(
                            f"Skipping invalid data: {post['userid']}, {post['relevant_user_id']} - {e}"
                        )
