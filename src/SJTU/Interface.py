...
class DDQNInterface(BaseModel):
    ...
    def __init__(
        ...
        user_map_dict: dict = None,
        user_dict_reverse: dict = None,
    ):
        super().__init__(
            budget=budget, user_feature=[], post_feature=[], selected_id_nodes=[]
        )
        self.user_map_dict = user_map_dict if user_map_dict is not None else {}
        self.user_dict_reverse = (
            user_dict_reverse if user_dict_reverse is not None else {}
        )
        self.budget = budget
