...
from src.ZJNU.data_structure import AccountInfo
...
class FeatureClusterSelector:
    ...
    def preprocess_data(self):
        ...
        self.norm_variable_numeric_stats_sample = (
            self.variable_numeric_stats_sample
            - self.variable_numeric_stats_sample.mean()
        ) / self.variable_numeric_stats_sample.std()
        # print("Data has been standardized (mean=0, std=1).")

        # 10. 清理数据（虽然已经删除了 NaN 和零方差列，但这里保留以防）
        self.norm_variable_numeric_stats_sample = self.clean_dataset(
            self.norm_variable_numeric_stats_sample
        )
