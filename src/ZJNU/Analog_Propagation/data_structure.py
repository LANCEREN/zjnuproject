...
class ICResult(BaseModel):
    ...
    activation_paths_info1: list[str]  # 记录I1传播的激活路径
    activation_paths_info2: list[str]  # 记录I2传播的激活路径
    step_activations_info1: list[str]  # 记录每步I1状态节点的激活信息
    step_activations_info2: list[str]  # 记录每步I2状态节点的激活信息

# 用户属性模型 用于策略生成的(作废)
# class User:
#     account_id: str  # 用户的账号 ID
#     personal_desc: torch.Tensor  # 用户的个人描述信息的嵌入向量
#     followers_count: int  # 用户的粉丝数
#     friends_count: int  # 用户的好友数
#     platform: int  # 用户所在平台的标识符
#
#     def __init__(self, account_id: str, personal_desc: torch.Tensor, followers_count: int, friends_count: int,
#                  platform: int):
...
# 用户订阅信息模型（作废）
# class UserSubscribeFeature(BaseModel):
#     account_id: str  # 账号的唯一标识符
#     target_account_id: str  # 目标订阅账号的 ID
