...
class InfluencePredictor:
    def __init__(
        ...
        static_path: str,
        dynamic_path: str,
        datatime1: datetime.date,
        datatime2: datetime.date,
        datatime3: datetime.date,
        datatime4: datetime.date,
    ):
    ...
    def predict_influence(self) -> np.ndarray[float]:
        ...
        parser.add_argument(
            "--lr", type=float, default=2e-3, help="Initial learning rate."
        )
        parser.add_argument(
            "--weight_decay",
            type=float,
        ...
        parser.add_argument(
            "--feat_dim", type=int, default=64, help="Feature dimension"
        )
        ...
        # 加载网络数据
        static_network, idx_map, idx = data_processor.load_networks_txt(
            self.static_path
        )
        dynamic_network = data_processor.load_networks_with_time_txt(
            self.static_path, self.dynamic_path, idx_map
        )
        dynamic1, dynamic2 = time_Windows.split_by_time(
            self.static_path,
            self.dynamic_path,
            idx_map,
            self.datatime1,
        ...
        # 生成节点嵌入
        print("生成静态网络节点嵌入...")
        static_embedding_data = node_embedding.generate_embeddings(
            static_network, args.feat_dim
        )
        ...
        print("生成第一个时间窗口节点嵌入...")
        dynamic1_embedding_data = node_embedding.generate_embeddings(
            dynamic1, args.feat_dim
        )
        ...
        print("生成第二个时间窗口节点嵌入...")
        dynamic2_embedding_data = node_embedding.generate_embeddings(
            dynamic2, args.feat_dim
        )
        ...
        static_features = np.genfromtxt(
            r"./Influence_Prediction/data/node_data.txt", dtype=float
        )
        print("提取第一个时间窗口群体特征...")
        # dynamic1_features = feature_extractor.extract_group_dynamic_features(dynamic1, 20)
        dynamic1_features = np.genfromtxt(
            r"./Influence_Prediction/data/node_data1.txt", dtype=float
        )
        ...
        dynamic2_features = np.genfromtxt(
            r"./Influence_Prediction/data/node_data2.txt", dtype=float
        )
        ...
        # 训练网络
        print("训练静态网络...")
        static_trainer = GATTrainer_Static(
            args, static_embedding_data, static_features, static_labels, static_network
        )
        ...
        print("训练第一个时间窗口网络...")
        dynamic1_trainer = GATTrainer_Dynamic1(
            args, dynamic1_embedding_data, dynamic1_features, dynamic1_labels, dynamic1
        )
        emb_dynamic1_with_idx = dynamic1_trainer.train()
        print("训练第二个时间窗口网络...")
        dynamic2_trainer = GATTrainer_Dynamic2(
            args,
            dynamic2_embedding_data,
            dynamic2_features,
            dynamic2_labels,
            dynamic2,
            emb_dynamic1_with_idx,
        )
        emb_dynamic2_with_idx = dynamic2_trainer.train()
        # 合并两个时间窗口嵌入
        emb_dynamic = np.concatenate(
            (emb_dynamic1_with_idx[:, 1:], emb_dynamic2_with_idx[:, 1:]), axis=1
        )
        # 训练注意力机制
        ...
        mlp_input = np.concatenate(
            (emb_static_with_idx[:, 1:], emb_dynamic_idx[:, 1:]), axis=1
        )
...
class Pridict_User_Influence:
    ...
    def predict_influence(self):
        ...
        dynamic1, dynamic2 = time_Windows.split_by_time(
            self.post_info_list, static_network
        )
        print(len(static_network.nodes()))
        print(len(dynamic_network.nodes()))
        print(len(dynamic1.nodes()))
        ...
        # 生成群体特征
        print("提取静态网络群体特征...")
        static_features = feature_extractor.extract_group_static_features(
            static_network, dynamic_network, 20
        )
        # static_features = np.column_stack((sorted(static_network.nodes()), np.ones((len(static_network.nodes()), 2))))
        print("提取第一个时间窗口群体特征...")
        dynamic1_features = feature_extractor.extract_group_dynamic_features(
            dynamic1, 20
        )
        # dynamic1_features = np.column_stack((sorted(static_network.nodes()), np.ones((len(static_network.nodes()), 1))))
        print("提取第二个时间窗口群体特征...")
        dynamic2_features = feature_extractor.extract_group_dynamic_features(
            dynamic2, 20
        )
        # dynamic2_features = np.column_stack((sorted(static_network.nodes()), np.ones((len(static_network.nodes()), 1))))
        # 生成标签
