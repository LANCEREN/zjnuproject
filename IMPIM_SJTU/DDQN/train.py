...
class MyTrainer():
    def __init__(self, device, training , T , lr , epoch, budget,data_dir):
        ...
        graph_train = graph_utils.read_graph(graph_path, node_feature_path, ind=0, directed=True)
        ...
        self.train_env = Environment(graph_train, self.budget, method='RR', use_cache=True , training=self.training)
        self.test_env = Environment(graph_test, self.budget, method='MC', use_cache=True , training=not(self.training))
        self.agent = Agent(self.device,self.reg_hidden,self.embed_dim,self.training,self.T,self.n_step,self.lr,self.bs,memory_size=self.memory_size)
    ...
    def My_train(self):
        ...
        if self.training:
            sample_size, clf_para, last_rewards = my_runner.train(self.epoch, self.model_file, self.reward_file , embed_file=self.embed_file)       
        else:
            sample_size, clf_para = my_runner.test(num_trials=10, embed_file=self.embed_file)
        return sample_size , clf_para, last_rewards

class Agent:
    def __init__(self, device, reg_hidden, embed_dim, training, T , n_step , lr, batch_size=64, memory_size=1000):
        self.gamma = 0.99
        self.node_dim = 7
        self.edge_dim = 3
        ...
        self.model = GNN_DDQN(reg_hidden=self.reg_hidden, embed_dim=self.embed_dim, node_dim=self.node_dim, edge_dim=self.edge_dim, T=self.T, w_scale=0.01, avg=False).to(self.device)
        if self.training:
            self.target = GNN_DDQN(reg_hidden=self.reg_hidden, embed_dim=self.embed_dim, node_dim=self.node_dim, edge_dim=self.edge_dim, T=self.T, w_scale=0.01, avg=False).to(self.device)
    ...
    def setup_graph_input(self, graph, states, actions=None,path=None):
        ...
        if id(graph) not in self.graph_node_embed:
            self.graph_node_embed[id(graph)] = node_embed(graph,self.device,self.node_dim,path) # epochs for initial embedding
        with torch.no_grad():
            # 节点特征
            ...
            # 将权重列表转换为tensor张量
            edge_attr[:, 1] = torch.tensor(list(graph.edges.values()), dtype=torch.float32)
            # 状态加入转换向量
    ...
    def setup_graph_batch_input(self, graph, states, actions=None,path=None):
        ...
        for i in range(sample_size):
            if id(graph[i]) not in self.graph_node_embed:
                self.graph_node_embed[id(graph[i])] = node_embed(graph[i],self.device,self.node_dim,path) # epochs for initial embedding
            with torch.no_grad():
                # 节点特征
                ...
                # 通过graphs[i].from_to_edges()获取边的索引，并将其转换为PyTorch张量edge_index
                edge_index = torch.from_numpy(graph[i].get_edge_index()).contiguous().long()
                
                # 初始化边特征矩阵，并根据边的权重和节点状态调整其值
                edge_attr = torch.ones(graph[i].num_edges(), self.edge_dim)
                # 将权重列表转换为tensor张量
                edge_attr[:, 1] = torch.tensor(list(graph[i].edges.values()), dtype=torch.float32)
                # 状态加入转换向量
                edge_attr[:, 0] = states[i][edge_index[0]]
                edge_attr[:, 2] = torch.abs(states[i][edge_index[0]] - states[i][edge_index[1]])
                y = actions[i].clone() if actions is not None else None
...
# 探索允许代理发现新的、可能更好的策略，而利用则使代理能够利用已知的最佳策略。通过调整epsilon值，可以控制探索与利用之间的平衡
    def select_action(self, graph, state, epsilon, training=True, budget=None,path=None):
        ''' act upon state '''
        ...
        if not(training):
            graph_input = self.setup_graph_input(graph, state.unsqueeze(dim=0),path=path)
            with torch.no_grad():
...
# 这种n步回放记忆机制有助于提高学习过程的稳定性和效率。
    ...
    def replay(self , path):
        ...
        state_action_values = self.model(self.setup_graph_batch_input(graph_batch, state_batch, action_batch , path = path)).squeeze(dim=1)
        next_state_values = torch.zeros(sample_size, device=self.device)

        if len(non_final_next_states) > 0:
            batch_non_final = self.setup_graph_batch_input(non_final_next_states_graphs, non_final_next_states, path = path)
...
# 在训练模式下，智能体会根据环境状态选择动作，并根据动作的结果更新其知识。在测试模式下，可以选择一次性生成动作序列，以评估智能体的性能。
    def play_game(self, num_iterations, epsilon, training=True, time_usage=False, one_time=False , path=None):
        ...
        for iteration in range(num_iterations):
            ...
            if training:
                ...
                for i in count():
                    ...
                    state = torch.tensor(self.env.state, dtype=torch.long).to(self.agent.device)
                    action = self.agent.select_action(self.env.graph, state, epsilon, training = self.training , path = path).item()
                ...
                if id(self.env.graph not in self.agent.graph_node_embed):
                    self.agent.graph_node_embed[id(self.env.graph)] = node_embed(self.env.graph,self.agent.device,7,path) # 20 = epochs for initial embedding , 7 = node_dim
                ...
                if one_time:
                    state = torch.tensor(self.env.state, dtype=torch.long).to(self.agent.device)
                    actions = self.agent.select_action(self.env.graph, state, epsilon, training=training, budget=self.env.budget , path = path).tolist()
                    ...
                    for i in count():
                        state = torch.tensor(self.env.state, dtype=torch.long).to(self.agent.device)
                        action = self.agent.select_action(self.env.graph, state, epsilon, training=training , path = path).item()
...
# train方法用于训练智能体。它首先进行预训练，然后在多个周期内调整智能体的行为策略，逐渐减少探索率并定期保存模型和更新目标网络。
    ...
    def test(self, num_trials , embed_file):
        ...
        all_rewards, all_seeds = self.play_game(num_trials, 0.0, False, time_usage = True, one_time = True , path=embed_file)
        ...
        all_rewards, all_seeds = self.play_game(num_trials, 0.0, False, time_usage = True, one_time = False,path=embed_file)
        print(f'Number of trials: {num_trials}')
        print(f'Graph path: {", ".join(g.path_graph for g in self.env.graph)}')
        cnt = 0
        for a_r, a_s in zip(all_rewards, all_seeds):
        ...
        if need_compute:
            if self.method == 'MC':
                with Pool(num_process) as p:
                    es_inf = statistics.mean(p.map(reward_fun.workerMC, 
                        [[self.graph, S, int(num_trial / num_process)] for _ in range(num_process)]))
            elif self.method == 'RR':
                if self.use_cache:
                    # cached without incremental
                    es_inf = reward_fun.computeRR(self.graph, S, num_trial, cache=self.RRs)
                else:
